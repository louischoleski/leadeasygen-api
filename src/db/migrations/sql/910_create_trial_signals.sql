...
--
--   'signup' — captured at POST /auth/register (user_id unknown yet, may stay
--              NULL): feeds the signup-velocity signals.
--   'trial'  — one row per gate decision at trial checkout; the card
--              fingerprint is back-filled by the subscription.created bus
--              subscriber once Stripe reports the card.
--
-- PII firewall: every correlating value is stored as sha256(pepper + value) —
-- never raw. Rows expire (expires_at, purged opportunistically by the gate);
-- legal basis is legitimate interest (fraud prevention), and nothing here may
-- flow into the pseudonymous analytics/telemetry pipeline.
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS trial_signals (
	...
	email_domain            TEXT,
	risk_score              INTEGER,
	decision                TEXT CHECK (decision IN ('granted', 'challenged', 'denied')),
